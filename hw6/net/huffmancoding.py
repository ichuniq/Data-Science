    ...
    for name, param in model.named_parameters():
        if 'weight' in name:
            ...
            if 'conv' in name:
                ...
                conv = param.data.cpu().numpy() # conv.dump(f'{directory}/{name}')
                
                data, data_len, indices, indptr = np.empty(0), np.empty(0), np.empty(0), np.empty(0)
                for i in range(shape[0]):
                    for j in range(shape[1]):
                        mat = csr_matrix(conv[i][j])
                        data = np.concatenate((data, mat.data))
                        data_len = np.concatenate((data_len, np.array([len(mat.data)]) ) )
                        indices = np.concatenate((indices, mat.indices))
                        indptr = np.concatenate((indptr, calc_index_diff(mat.indptr)))
 
                data = data.astype('float32')
                data_len = data_len.astype('int32')
                indices = indices.astype('int32')
                indptr = indptr.astype('int32')
                form = 'csr'

                # Encode
                t0, d0 = huffman_encode(data, name+f'_{form}_data', directory)
                t1, d1 = huffman_encode(data_len, name+f'_{form}_datalen', directory)
                t2, d2 = huffman_encode(indices, name+f'_{form}_indices', directory)
                t3, d3 = huffman_encode(indptr, name+f'_{form}_indptr', directory)
    ...
    for name, param in model.named_parameters():
        if 'weight' in name:
            ...
            if 'conv' in name:
                ...
                data_len = huffman_decode(directory, name+f'_{form}_datalen', dtype='int32')
                ...
                for i in range(shape[0]):
                    for j in range(shape[1]):
                        _range = data_len[i * shape[1] + j]
                        ri = reconstruct_indptr(indptr[st_indptr : st_indptr+shape[2]])
                        # print(ri)
                        conv = matrix((data[start : start+_range], indices[start : start+_range], ri), shape=(shape[2], shape[3]))
                        param.data[i][j] = torch.from_numpy(conv.toarray()).to(dev)
                        start += _range
                        st_indptr += shape[2]
